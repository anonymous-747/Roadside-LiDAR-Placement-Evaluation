...
def create_voxel_heatmap(region, label_folder):
    ...
    # # Use imshow to create the heatmap. 'viridis' is a good colormap for visibility.
    # # The 'origin' is set to 'lower' so that (0,0) is at the bottom-left.
    # im = ax.imshow(voxel_grid_sum, cmap='pink', interpolation='nearest', origin='lower', 
    #                 extent=[x_start, x_end, y_start, y_end])

    # # Add a color bar to show the scale of the counts
    # cbar = fig.colorbar(im, ax=ax)
    # cbar.set_label('Number of Points per Voxel', rotation=270, labelpad=20, color='black')
    # plt.setp(plt.getp(cbar.ax.axes, 'yticklabels'), color='black')


    # # Set plot titles and labels with black color for readability on a white background
    # ax.set_title('Voxel Heatmap', fontsize=18, color='black', pad=20)
    # ax.set_xlabel('X-coordinate (meters)', fontsize=12, color='black')
    # ax.set_ylabel('Y-coordinate (meters)', fontsize=12, color='black')

    # # Set the background color of the plotting area itself
    ...
    # # Set grid lines corresponding to the voxels. Use a light gray for the grid.
...
if __name__ == '__main__':
    # --- Configuration ---
    # 1. Define the region of interest: [x_start, y_start, x_end, y_end]
    # Example: A 20m x 20m area starting from (0,0)
    region_of_interest = [-160, -160, 160, 160]

    # 2. Specify the folder containing your label files.
    # IMPORTANT: Create a folder named 'labels' in the same directory as this script,
    # or change this path to your folder's location.
    labels_directory = '/home/qzhq747/data/crossroad/pos1/vehicle_num'

    # --- Create dummy data for demonstration if the folder doesn't exist ---

    # --- Run the main function ---
    create_voxel_heatmap(region_of_interest, labels_directory)
